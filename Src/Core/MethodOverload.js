...
import SystemException from "../Exception/SystemException.js";
...
export default function () {
    ...
    MethodOverload.Add = function (typeArr, method) {
        if (!Array.isArray(typeArr)) {
            throw new ArgumentTypeException("typeArr", "Array");
        }

        if (typeof method !== "function") {
            throw new ArgumentTypeException("method", "Function");
        }
        ...
        for (let i = 0; i < typeArr.length; i++) {
            ...
            if (typeof typeObj !== "function" && !isArray && '*' !== typeObj) {
                throw new SystemException(`预判类型必须是一个 Class 或 Array。`);
            }

            if (isArray) {
                for (let n = 0; n < typeObj.length; n++) {
                    if (typeof typeObj[n] !== "function" && typeObj[n] !== null && '*' !== typeObj[n]) {
                        throw new SystemException("预判类型列举内容必须是一个 Class 、 null 或 *。");
                    }
                }
            }
        }
...
function MatchType(param, type) {
    ...
    if (typeof type !== "function") {
        ...
        if (param !== null && param["#isCollection"]) {
            return param.GetType() === type["#internalType"] || (typeof type["#internalType"] === "undefined" && param instanceof Collection);
        }
